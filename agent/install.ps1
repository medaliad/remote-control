...
$nodeArg = "`"$InstallDir\agent.js`""
# `pwsh -WindowStyle Hidden` gives us a windowless launcher. We pipe stdout/
# stderr to agent.log under %LocalAppData% so support can ask the user to
# attach it when something goes wrong, without keeping a console open.
$launcher = @(
    "-NoProfile",
    "-WindowStyle", "Hidden",
    "-Command",
    "& '$nodeExe' '$InstallDir\agent.js' *>> '$LogPath'"
) -join " "

$action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument $launcher -WorkingDirectory $InstallDir
