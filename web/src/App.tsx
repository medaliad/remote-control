...
function parseLocation(): ParsedLocation {
  ...
  let route: Route;
  if (path === "/host") {
    route = { kind: "host", autoPairToken };
  } else if (path === "/client") {
    route = { kind: "client", prefillCode: qs.get("code"), autoPairToken };
  } else {
    route = { kind: "home" };
  }
  return { route, embed };
...
export function App() {
  // Subscribe so the whole tree re-renders on locale change.
  useLocale();
      ...
      <div className="relative min-h-screen w-full flex flex-col bg-canvas overflow-hidden">
        <main className="flex-1 flex w-full">
          <div key={route.kind} className="w-full flex-1 flex animate-fade-in">
            ...
            {route.kind === "host" && (
              <HostPage embed autoPairToken={route.autoPairToken} />
            )}
            {route.kind === "client" && (
              <ClientPage
                prefillCode={route.prefillCode}
                autoPairToken={route.autoPairToken}
                embed
              />
            )}
          </div>
        ...
        <div className="max-w-7xl mx-auto flex items-center gap-2 sm:gap-3 px-3 sm:px-5 md:px-8 py-3 sm:py-3.5">
          ...
          >
            ...
            <span className="flex flex-col leading-tight min-w-0">
              <span className="text-[14px] sm:text-[15px] font-bold truncate text-text900">
                {t("nav.brand")}
              </span>
          ...
          >
            ...
            <span className="hidden xs:inline">{t("nav.host")}</span>
          </a>
          <a
            className={[
              "relative inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200",
          ...
          >
            ...
            <span className="hidden xs:inline">{t("nav.join")}</span>
          </a>
        </div>
      </nav>
