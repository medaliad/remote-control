...
const AUTOPAIR_ALLOWED_ORIGINS = (process.env.AUTOPAIR_ALLOWED_ORIGINS ?? "*").split(",").map(s => s.trim()).filter(Boolean);
const __dirname = fileURLToPath(new URL(".", import.meta.url));
const WEB_ROOT = process.env.WEB_ROOT ? resolve(process.env.WEB_ROOT) : resolve(__dirname, "..", "..", "web", "dist");
...
const httpServer = createServer(async (req, res) => {
  ...
  }
  {
    ...
    if (isInstallPath(pathname)) {
      ...
      const host = (req.headers["x-forwarded-host"] as string | undefined)?.split(",")[0]?.trim() ?? req.headers.host as string | undefined ?? "remote-control-cdqo.onrender.com";
...
async function handleApi(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (path === "/api/agent/lookup") {
    ...
    if (!authorize(req, url)) return writeJson(res, 401, {
      ok: false,
      error: "unauthorized"
    });
  ...
  if (path === "/api/users/login") {
    ...
    if (!user) return writeJson(res, 400, {
      ok: false,
      error: "missing-user"
    });
...
async function openSessionRoute(req: IncomingMessage, res: ServerResponse, url: URL): Promise<void> {
  ...
  const body = await readJsonBody(req);
  if (!body) return writeJson(res, 400, {
    ok: false,
    error: "bad-json"
  });
  ...
  if (!targetUser) return writeJson(res, 400, {
    ok: false,
...
function writeCors(req: IncomingMessage, res: ServerResponse): void {
  ...
  const allow = AUTOPAIR_ALLOWED_ORIGINS.includes("*") ? "*" : AUTOPAIR_ALLOWED_ORIGINS.includes(origin) ? origin : "";
          ...
          if (!user || !agentId) {
            try {
              ws.send(JSON.stringify({
                type: "error",
                code: "bad-message",
                message: "user/agentId required"
              }));
            } catch {}
            return;
          }
          const entry = sessions.registerAgent(ws, user, agentId);
          const reply: ServerToAgent = {
    ...
    case "host:claim":
      {
        ...
        if (!r.ok) {
          send(ws, {
            type: "error",
            code: "invalid-token",
            message: "Pairing token unknown or expired."
          });
          return;
        }
        ...
        if (r.pairedClient) {
          ...
          send(r.session.hostWs, {
            type: "peer:ready",
            role: "host",
            allowControl: r.session.allowControl
          });
          send(r.pairedClient, {
            type: "peer:ready",
            role: "client",
    ...
    case "host:reject":
      {
        ...
        send(r.clientWs, {
          ...
          reason: msg.reason ?? "rejected by host"
        });
        try {
          r.clientWs.close(1000, "rejected");
        } catch {}
        return;
      }
    case "host:end":
      {
        ...
        if (s.clientWs) {
          ...
          try {
            s.clientWs.close(1000, "host ended");
          } catch {}
        }
    ...
    case "host:setControl":
      {
        ...
        if (s.clientWs) send(s.clientWs, {
          type: "control:changed",
          allowed: s.allowControl
        });
        return;
      }
    case "client:join":
...
function errorMessage(code: string): string {
  switch (code) {
    ...
    case "session-full":
      return "Session already has a connected client.";
    case "not-host":
      return "Only the host can do that.";
    case "not-client":
      return "Only a client can do that.";
    case "no-pending-request":
      return "No such pending request.";
    case "not-paired":
      return "Not paired with a peer yet.";
    case "invalid-token":
      return "Pairing token unknown or expired.";
    default:
